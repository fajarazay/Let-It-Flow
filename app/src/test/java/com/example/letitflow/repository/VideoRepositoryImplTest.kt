...
class VideoRepositoryImplTest {
    ...
    @Test
    fun `fetchVideoList should not completed when failed`() = runBlocking{
        val expectedError = Throwable("error")

        //coEvery { mockedVideoService.fetchVideoList()} returns  { Throwable("")}
        whenever(mockedVideoService.fetchVideoList()).thenThrow(expectedError)

        videoRepository.fetchVideoList().test {
            assertEquals(expectedError, awaitError())
        }
    }
}
